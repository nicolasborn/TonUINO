...
{
    public:
        // AnalogButton(pin, dbTime, puEnable, invert) instantiates a button object.
        //
        // Required parameter:
        ...
        AnalogButton(uint8_t buttonNr, uint32_t dbTime=25)
            : m_buttonNr(buttonNr), m_dbTime(dbTime) {}
        ...
        bool read(uint8_t pressedButtonNr);
    ...
    private:
        uint8_t m_buttonNr;          // arduino pin number connected to button
...
class ToggleButton : public AnalogButton
{
    public:
    
        // constructor is similar to Button, but includes the initial state for the toggle.
        ToggleButton(uint8_t buttonNr, bool initialState=false, uint32_t dbTime=25)
            : AnalogButton(buttonNr, dbTime), m_toggleState(initialState) {}

        // read the button and return its state.
        // should be called frequently.
        bool read(uint8_t pressedButtonNr)
        {
            AnalogButton::read(pressedButtonNr);
            if (wasPressed())
            {
