...
void AnalogButton::begin()
{
    ...
    m_state = false; //digitalRead(m_pin);
    m_time = millis();
    m_lastState = m_state;
    m_changed = false;
    m_lastChange = m_time;
...
/-----------------------------------------------------------------------*/
bool AnalogButton::read(uint8_t pressedButtonNr)
{
    uint32_t ms = millis();
    bool pinVal = m_buttonNr == pressedButtonNr;
 ...
 * These functions do not cause the button to be read.                  *
 *----------------------------------------------------------------------*/
bool AnalogButton::pressedFor(uint32_t ms)
{
    return m_state && m_time - m_lastChange >= ms;
}

bool AnalogButton::releasedFor(uint32_t ms)
{
    return !m_state && m_time - m_lastChange >= ms;
}
 ...
 * in milliseconds.                                                     *
 *----------------------------------------------------------------------*/
uint32_t AnalogButton::lastChange()
{
    return m_lastChange;
}
